...
for j = 1:length(velocity)
    ...
    for i = phase
        nodeLength = size(solution.states{i}.x,2);
        AveVelocity = 0.1*velocity(j)*ones(1,nodeLength);
        
        figure(1); hold on; grid on
        plot3(AveVelocity, solution.states{i}.dx(1,:), solution.states{i}.x(5,:),'linewidth',2,'color',color(i,:));
    end
end
