function virtual_constraints(nlp, bounds, load_path)
    ...
    domain = nlp.Plant;                                 % MARLO   
    T  = SymVariable('t',[2, 1]);                       % '{{t11}, {t21}}'
    field_names = fieldnames(domain.VirtualConstraints);% {'output'}
    for i = 1:length(field_names)
        ...
        p_name = domain.VirtualConstraints.(field_names{i}).PhaseParamName;% 'poutput'
        p = {SymVariable(tomatrix(domain.VirtualConstraints.(field_names{i}).PhaseParams(:)))};% {'{{p$1$1}, {p$2$1}}'}
